...
import com.twilio.chat.app.testUtil.BottomSheetBehaviorIdlingResource
import com.twilio.chat.app.testUtil.WaitForViewMatcher
import com.twilio.chat.app.testUtil.atPosition
import com.twilio.chat.app.testUtil.removeProgressBarIndeterminateDrawables
...
class ChannelActivityTest {
    ...
    private val plainMember = MemberDataItem(
        identity = "", channelSid = "", lastConsumptionTimestamp = null,
        lastConsumedMessageIndex = null, sid = "", friendlyName = "user", isOnline = true
    )

    @get:Rule
    var activityRule: IntentsTestRule<ChannelActivity> = IntentsTestRule(ChannelActivity::class.java, false, false)
    ...
    @Before
    fun setUp() {
        activityRule.launchActivity(ChannelActivity.getStartIntent(InstrumentationRegistry.getInstrumentation().targetContext, channelSid))
        ChatClientWrapper.recreateInstance()
        channelViewModel = activityRule.activity.channelViewModel
    ...
    fun incomingMessagesDisplayed() {
        val messages = getMockedMessages(MESSAGE_COUNT, messageBody,
            channelSid, Direction.INCOMING.value, messageAuthor).asMessageListViewItems()
        testInjector.messageResultChannel.sendBlocking(RepositoryResult(messages.asPagedList(), RepositoryRequestStatus.COMPLETE))
    ...
    fun outgoingMessagesDisplayed() {
        val messages = getMockedMessages(MESSAGE_COUNT, messageBody,
            channelSid, Direction.OUTGOING.value, messageAuthor, sendStatus = SendStatus.SENT).asMessageListViewItems()
        testInjector.messageResultChannel.sendBlocking(RepositoryResult(messages.asPagedList(), RepositoryRequestStatus.COMPLETE))
    ...
    fun typingIndicatorDisplayed() {
        testInjector.typingMembersListChannel.offer(listOf(plainMember.copy(identity = "user1")))
        onView(withId(R.id.typingIndicator)).check(matches(withText("user1 is typing…")))
    }

    @Test
    fun typingIndicatorMaxLength() {
        testInjector.typingMembersListChannel.offer(
            listOf(
                plainMember.copy(identity = "user1"),
                plainMember.copy(identity = "user2"),
                plainMember.copy(identity = "user3"),
                plainMember.copy(identity = "user4")
            )
        )
    ...
    fun incomingMessagesReactionsDisplayed() {
        ...
        val messages = getMockedMessages(MESSAGE_COUNT, messageBody,
            channelSid, Direction.INCOMING.value, messageAuthor, attributes).asMessageListViewItems()
        testInjector.messageResultChannel.sendBlocking(RepositoryResult(messages.asPagedList(), RepositoryRequestStatus.COMPLETE))
    ...
    fun incomingMessagesReactionsAddedAndRemoved() {
        val memberSid = "member2"
        var reactionAttributes = listOf(Reaction.HEART).getExpectedReactions(listOf(memberSid))
        var attributes = Gson().toJson(reactionAttributes)
        var message = getMockedMessages(1, messageBody,
            channelSid, Direction.INCOMING.value, messageAuthor, attributes).asMessageListViewItems()
        testInjector.messageResultChannel.sendBlocking(RepositoryResult(message.asPagedList(), RepositoryRequestStatus.COMPLETE))

        validateMessageItems(message)

        reactionAttributes = listOf(Reaction.HEART).getExpectedReactions(listOf(memberSid, messageAuthor))
        ...
        reactionAttributes = listOf(Reaction.HEART).getExpectedReactions(listOf(memberSid))
        attributes = Gson().toJson(reactionAttributes)
        message = getMockedMessages(1, messageBody,
            channelSid, Direction.INCOMING.value, messageAuthor, attributes).asMessageListViewItems()
        testInjector.messageResultChannel.sendBlocking(RepositoryResult(message.asPagedList(), RepositoryRequestStatus.COMPLETE))
    ...
    fun outgoingMessagesReactionsDisplayed() {
        ...
        val attributes = Gson().toJson(reactionAttributes)
        val messages = getMockedMessages(MESSAGE_COUNT, messageBody,
            channelSid, Direction.OUTGOING.value, messageAuthor, attributes, sendStatus = SendStatus.SENT).asMessageListViewItems()
        testInjector.messageResultChannel.sendBlocking(RepositoryResult(messages.asPagedList(), RepositoryRequestStatus.COMPLETE))
    ...
    fun outgoingMessagesReactionsAddedAndRemoved() {
        ...
        var attributes = Gson().toJson(reactionAttributes)
        var message = getMockedMessages(1, messageBody,
            channelSid, Direction.OUTGOING.value, messageAuthor, attributes).asMessageListViewItems()
        testInjector.messageResultChannel.sendBlocking(RepositoryResult(message.asPagedList(), RepositoryRequestStatus.COMPLETE))
    ...
    fun incomingMediaMessageDisplayed() {
        val messages = getMockedMessages(1, messageBody,
            channelSid, Direction.INCOMING.value, messageAuthor, type = Message.Type.MEDIA,
    ...
    fun outgoingMediaMessageDisplayed() {
        val messages = getMockedMessages(1, messageBody,
            channelSid, Direction.OUTGOING.value, messageAuthor, type = Message.Type.MEDIA,
            mediaFileName = "test.txt", mediaSize = 100).asMessageListViewItems()
        testInjector.messageResultChannel.sendBlocking(RepositoryResult(messages.asPagedList(), RepositoryRequestStatus.COMPLETE))
    ...
    private fun validateMessageItems(messages: List<MessageListViewItem>) =
        messages.forEachIndexed { index, channel ->
            validateMessageItem(index, channel)
        }
