...
interface MessagesDao {
    ...
    @Query("SELECT * FROM message_table WHERE channelSid = :channelSid ORDER BY CASE WHEN `index` < 0 THEN dateCreated ELSE `index` END ASC")
    fun getMessagesSorted(channelSid: String): DataSource.Factory<Int, MessageDataItem>
